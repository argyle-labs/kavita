//! kavita service backend — Manga/ebook/comics server.
//!
//! Implements `ServiceBackend` so the generic `service.*` tools
//! (deploy/backup/restore/configure/status/connect/sync) drive kavita. No
...
impl ServiceBackend for KavitaBackend {
    fn provider(&self) -> &str { self.provider }

    /// Runtimes kavita can be placed on. `service.deploy` hands the
    ...
    {
        // TODO: describe the kavita workload (image/template, ports, mounts,
        ...
        Box::pin(async move { Err(ServiceError::unimplemented("kavita.workload_spec")) })
    }
        ...
        -> BoxFuture<'a, Result<BackupArtifact, ServiceError>>
    {
        ...
        Box::pin(async move { Err(ServiceError::unimplemented("kavita.backup")) })
    }

    fn restore<'a>(&'a self, _ep: &'a Endpoint, _from: &'a BackupArtifact)
        -> BoxFuture<'a, Result<(), ServiceError>>
    {
        Box::pin(async move { Err(ServiceError::unimplemented("kavita.restore")) })
    }

    fn configure<'a>(&'a self, _ep: &'a Endpoint, _config: &'a str)
        -> BoxFuture<'a, Result<(), ServiceError>>
    {
        // TODO: apply kavita-specific config idempotently.
        Box::pin(async move { Err(ServiceError::unimplemented("kavita.configure")) })
    }
        ...
        -> BoxFuture<'a, Result<ServiceStatus, ServiceError>>
    {
        ...
        Box::pin(async move { Err(ServiceError::unimplemented("kavita.status")) })
    }
}
...
mod tests {
    ...
    fn declares_provider() {
        let b = KavitaBackend::new("kavita");
        assert_eq!(b.provider(), "kavita");
    }
}
